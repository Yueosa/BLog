...
- **⚠️ 安全警告:** 在生产环境中，将 `listen_addresses` 设为 `*` 并放行 `0.0.0.0/0` 具有潜在风险。请务必：
    - 设置极其复杂的数据库密码。
    - 在云服务商后台（如腾讯云/阿里云）配置 **安全组规则**，仅放行你自己的开发 IP 或应用服务器 IP。
    - 如果应用和数据库在同一台服务器，建议优先使用 `127.0.0.1` 配合 `md5`，不要开启远程访问。
